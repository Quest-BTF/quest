...
export const HOUSE_MASTER_QUESTIONS = {
  cq1: {
    title: 'The Council\'s Questions',
    text: 'A builder in your house stops shipping and goes quiet. Your first move is to...',
    options: {
  ...
  cq2: {
    title: 'The Council\'s Questions',
    text: 'The trait you respect most in a builder is...',
    options: {
  ...
  cq3: {
    title: 'The Council\'s Questions',
    text: 'Under pressure, your own instinct is to...',
    options: {
...
function buildHouseMasterUserPrompt(masterData, tallyResult) {
  const { name, motivation, councilAnswers, decidingAnswer, decidingReason } = masterData;
  const allAnswers = { ...councilAnswers, decidingAnswer };
...
export async function evaluateHouseMaster(masterData, takenHouses = []) {
  ...
  const tallyAnswers = { ...masterData.councilAnswers, decidingAnswer: masterData.decidingAnswer };
  const tallyResult = calculateTally(tallyAnswers);
